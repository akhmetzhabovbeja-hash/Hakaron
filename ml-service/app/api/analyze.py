...
- Будь СТРОГИМ оценщиком
- Ответ в виде точки ".", пустой или из одного слова = 0 баллов за этот вопрос
- Если больше половины вопросов без ответа — total_score НЕ МОЖЕТ быть выше 40
- Если ответ начинается с "Ответ Gemini", "Ответ ChatGPT" или выглядит скопированным из AI — ШТРАФ -20 баллов по категории и отметь это в explanation
- Конкретные примеры с цифрами, именами, датами = высокий балл
- Общие слова без деталей = низкий балл
- strengths — ОБЯЗАТЕЛЬНО укажи 2-4 пункта (даже если кандидат слабый, найди что-то)
- weaknesses — ОБЯЗАТЕЛЬНО укажи 2-3 пункта
- summary — ОБЯЗАТЕЛЬНО напиши развёрнутое резюме 3-5 предложений
"""
